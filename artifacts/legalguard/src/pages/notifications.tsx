...
export default function Notifications() {
  ...
  const handleMarkAllRead = () => {
    ...
    // For now, we simulate it by iterating over unread.
  ...
  const getIcon = (category: string) => {
    switch(category.toLowerCase()) {
      ...
      case 'audit': return <History className="h-5 w-5 text-success" />;
                ...
                {notifications?.map((notification) => (
                  <div 
                    ...
                    className={`p-4 flex gap-4 transition-colors ${
                      !notification.isRead ? 'bg-primary/5' : 'bg-card hover:bg-muted/50'
                    }`}
                  >
